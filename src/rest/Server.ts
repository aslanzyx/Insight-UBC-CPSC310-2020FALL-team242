...
import { IInsightFacade, InsightDataset, InsightError, NotFoundError } from "../controller/IInsightFacade";
...
export default class Server {
    ...
    private static controller: IInsightFacade = new InsightFacade();
    ...
    private static listDatasets(req: restify.Request, res: restify.Response, next: restify.Next) {
        ...
        try {
            Server.controller.listDatasets().then((sol: InsightDataset[]) => {
                Log.info("Server::listDatasets(..) - responding " + 200);
                res.json(200, { result: sol });
            }).catch((err) => {
                Log.error("Server::listDatasets(..) - responding 400");
    ...
    private static addDataset(req: restify.Request, res: restify.Response, next: restify.Next) {
        ...
        try {
            Server.controller.addDataset(req.params.id, req.body.toString("base64"), req.params.kind)
                .then((soln: any) => {
                    Log.info("Server::addDataset(..) - responding " + 200);
                    res.json(200, { result: soln });
                }).catch((err) => {
    ...
    private static removeDataset(req: restify.Request, res: restify.Response, next: restify.Next) {
        ...
        try {
            Server.controller.removeDataset(req.params.id).then((sol: string) => {
                res.json(200, { result: sol });
    ...
    private static performQuery(req: restify.Request, res: restify.Response, next: restify.Next) {
        ...
        try {
            ...
            Server.controller.performQuery(req.params).then((sol: any[]) => {
